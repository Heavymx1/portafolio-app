...
def obtener_datos_mercado(tickers):
    ...
    for t_original in tickers:
        ...
        # Usamos el ticker ya limpio que viene del DataFrame agrupado
...
if df_raw is not None and not df_raw.empty:
    # 1. NORMALIZACIÓN DE COLUMNAS (Ignora PPP, Impuestos, etc.)
    df_raw.columns = df_raw.columns.str.lower().str.strip()
    
    # Solo nos importan estas columnas. Las demás las ignoramos.
    mapa_gbm = {
    ...
    # Limpiamos el Ticker ANTES de agrupar para que "AMZN" y "AMZN *" sean lo mismo
    df_raw['Ticker'] = df_raw['Ticker'].astype(str).str.replace('*', '', regex=False).str.replace(' N', '', regex=False).str.strip()
    
    # Limpiamos números (GBM usa $ y ,)
    def clean_money(x): 
    ...
    # 3. CÁLCULO CONTABLE (EL FIX MATEMÁTICO) 🧮
    # Calculamos cuánto costó CADA fila individualmente
    df_raw['Inversion_Fila'] = df_raw['Cantidad'] * df_raw['Costo_Unitario']
    
    # Agrupamos por Ticker limpio. Sumamos Cantidades y Sumamos Dinero Invertido.
    # NO sumamos el costo unitario, eso estaría mal.
    df = df_raw.groupby('Ticker', as_index=False).agg({
        'Tipo': 'first', 
        'Sector': 'first', 
        'Cantidad': 'sum', 
        'Inversion_Fila': 'sum', # Suma total de dinero invertido
        'Notas': 'first'
    })
    ...
    # Recalculamos el Costo Promedio REAL ponderado
    ...
    # Valor Mercado = Cantidad * Precio Hoy
    df['Valor_Mercado'] = df['Cantidad'] * df['Precio_Actual']
    
    # Ganancia = Valor Mercado - Lo que realmente gastaste (Inversion_Total_Real)
    ...
    # Filtros
    df_real = df[df['Cantidad'] > 0.001].copy() # Filtro > 0.001 para evitar errores de coma flotante
    ...
    with tab1:
        ...
        if not historia.empty:
            ...
            fig_hist.update_xaxes(rangeslider_visible=False, rangeselector=dict(buttons=list([dict(count=1, label="1M", step="month", stepmode="backward"), dict(count=3, label="3M", step="month", stepmode="backward"), dict(step="all", label="Todo")])))
        ...
        def bloque(titulo, tipo):
            ...
            if not d.empty:
                ...
                # Tabla con columnas contables claras
    ...
    st.info("Cargando...")import streamlit as st
import pandas as pd
import yfinance as yf
import plotly.express as px
import gspread
...
# --- CONEXIÓN ---
# 👇👇👇 ¡TU LINK AQUÍ! 👇👇👇
URL_HOJA = "AQUI_PEGA_TU_LINK_LARGO_COMPLETO" 
# 👆👆👆👆👆👆👆👆👆👆👆👆👆👆👆👆
